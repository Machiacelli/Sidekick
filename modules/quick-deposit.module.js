...
const QuickDepositModule = {
    ...
    async init() {
        ...
        try {
            ...
            this.settings.target = this.normalizeTarget(targetStored);

            // Older settings pages saved labels such as "Faction Vault"
            // instead of the canonical value used by the module.
            if (targetStored !== this.settings.target) {
                await ChromeStorage.set('quickDeposit_target', this.settings.target);
            }
            ...
            const isEnabled = moduleSettings.isEnabled === true;
    ...
    updateButtonText() {
        ...
        const target = this.normalizeTarget(this.settings.target);
    ...
    async executeDeposit(mode = 'auto') {
        ...
        const target = this.normalizeTarget(
            mode === 'auto' ? this.settings.target : mode
        );
    ...
    async updateSettings(newSettings) {
        if (newSettings.target) {
            const target = this.normalizeTarget(newSettings.target);
