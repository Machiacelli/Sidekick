...
.sk-prev-close{
    width:32px;height:32px;background:#353535;border:1px solid rgba(255,255,255,.34);
    border-radius:50%;color:#fff;font-size:23px;font-weight:400;cursor:pointer;
    display:flex;align-items:center;justify-content:center;transition:all .15s;line-height:1;
    opacity:1;text-shadow:0 1px 2px rgba(0,0,0,.8);flex-shrink:0;
}
.sk-prev-close:hover{background:#8b2f2f;color:#fff;border-color:#ff7777;transform:scale(1.06);}
.sk-prev-close:focus-visible{outline:2px solid var(--green);outline-offset:2px;}
.sk-subtab-bar{
...
/* Trading settings */
#skp-trading .sk-scroll{padding:18px 24px;}
    ...
    <div class="sk-sec-page active" id="skp-general">
      ...
      <div class="sk-scroll">
        ...
        <div class="sk-subtab-panel" id="skp-tab-notifications">
          ...
          <div class="sk-row"><div class="sk-row-info"><div class="sk-row-title">Notification Sounds</div><div class="sk-row-desc">Play a sound when in-page notifications appear</div></div><label class="sk-tog"><input type="checkbox" id="skp-notif-sound" checked><div class="sk-tog-track"></div><div class="sk-tog-thumb"></div></label></div>
          <div class="sk-row"><div class="sk-row-info"><div class="sk-row-title">Auto-dismiss</div><div class="sk-row-desc">Automatically hide notifications after a timeout</div></div><label class="sk-tog"><input type="checkbox" id="skp-notif-auto-dismiss" checked><div class="sk-tog-track"></div><div class="sk-tog-thumb"></div></label></div>
          <div class="sk-sh" style="margin-top:14px;">Notification Duration</div>
          <div class="sk-slider-row"><input type="range" min="2" max="10" value="5" id="skp-notif-duration" class="skp-slider" data-out="skp-notif-val" data-suffix="s"><span class="sk-slider-val" id="skp-notif-val">5s</span></div>
          ...
          <div class="sk-status" id="skp-notif-status" style="margin-top:10px;text-align:center;">Settings loaded</div>
        </div>
      </div>
    </div>
    ...
    <div class="sk-sec-page" id="skp-profile">
      ...
      <div class="sk-scroll">
        ...
        <div class="sk-subtab-panel" id="skp-tab-money">
          ...
          <select class="sk-select" id="skp-deposit-target"><option value="FACTION">Faction Vault</option><option value="PROPERTY">Property Vault</option><option value="COMPANY">Company Vault</option><option value="GHOST">Ghost Trade</option></select>
          <div class="sk-ghost-trade-row" id="skp-ghost-row" style="display:none;">
            ...
            <input type="text" class="sk-input" id="skp-ghost-id" placeholder="No ghost trade detected yet" readonly style="opacity:0.55;cursor:not-allowed;">
            <button class="sk-btn sk-btn-danger" id="skp-clear-ghost" style="width:100%;">Clear Ghost Trade ID</button>
          </div>
          <div class="sk-status" id="skp-quickdeposit-status" style="margin-top:10px;text-align:center;">Settings loaded</div>
...
`;
            ...
            overlay.querySelectorAll('.sk-nav-icon-img').forEach(img => {
                ...
                // Keep Trading usable even when the standalone icon was not copied
                // into assets/icons or its filename casing differs on another system.
                if (icon === 'Trading.png') {
                    const tradingFallbackSvg = `
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 154 154">
                            <defs>
                                <linearGradient id="skTradingGradient" x1="12" y1="12" x2="142" y2="142" gradientUnits="userSpaceOnUse">
                                    <stop offset="0" stop-color="#17e94f"/>
                                    <stop offset="0.52" stop-color="#9fd33a"/>
                                    <stop offset="1" stop-color="#ff9b43"/>
                                </linearGradient>
                            </defs>
                            <rect x="2" y="2" width="150" height="150" rx="13" fill="none" stroke="url(#skTradingGradient)" stroke-width="4"/>
                            <path d="M34 48H104V35L128 57L104 79V66H34C27 66 22 62 22 57C22 52 27 48 34 48Z" fill="url(#skTradingGradient)"/>
                            <path d="M120 88H50V75L26 97L50 119V106H120C127 106 132 102 132 97C132 92 127 88 120 88Z" fill="url(#skTradingGradient)"/>
                        </svg>`;
                    const tradingFallbackUrl = 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(tradingFallbackSvg);
                    img.addEventListener('error', () => {
                        if (img.src !== tradingFallbackUrl) img.src = tradingFallbackUrl;
                    });
                }
        ...
        async attachNotificationsTabListeners(panel) {
            const notifSoundCheckbox = panel.querySelector('#skp-notif-sound');
            const notifAutoDismissCheckbox = panel.querySelector('#skp-notif-auto-dismiss');
            const notifDurationSlider = panel.querySelector('#skp-notif-duration');
            const notifDurationDisplay = panel.querySelector('#skp-notif-val');
            const notifWindowsCheckbox = panel.querySelector('#skp-tog-browser-notif');
            const notifStatusDiv = panel.querySelector('#skp-notif-status');

            if (!notifSoundCheckbox || !notifAutoDismissCheckbox || !notifDurationSlider) return;

            // This key is intentionally separate from notification history.
            ...
            if (notifWindowsCheckbox) {
                notifWindowsCheckbox.checked = notifSettings.windowsNotifications === true;
            }
            const durationSeconds = Math.min(10, Math.max(2, Math.round((notifSettings.duration || 5000) / 1000)));
            notifDurationSlider.value = String(durationSeconds);
            if (notifDurationDisplay) notifDurationDisplay.textContent = `${durationSeconds}s`;
            ...
            const autoSaveNotifSettings = async () => {
                const settings = {
                    ...
                    windowsNotifications: notifWindowsCheckbox?.checked === true,
                    duration: parseInt(notifDurationSlider.value, 10) * 1000
                };
            ...
            notifSoundCheckbox.addEventListener('change', autoSaveNotifSettings);
            notifAutoDismissCheckbox.addEventListener('change', autoSaveNotifSettings);
            notifDurationSlider.addEventListener('input', () => {
                if (notifDurationDisplay) notifDurationDisplay.textContent = `${notifDurationSlider.value}s`;
            });
        ...
        async attachQuickDepositTabListeners(panel) {
            ...
            const ghostIdInput = panel.querySelector('#skp-ghost-id');
            const clearGhostBtn = panel.querySelector('#skp-clear-ghost');
            const ghostRow = panel.querySelector('#skp-ghost-row');
            const statusDiv = panel.querySelector('#skp-quickdeposit-status');
            ...
            try {
                ...
                if (targetStored) {
                    const normalizedTargets = {
                        'FACTION VAULT': 'FACTION',
                        'PROPERTY VAULT': 'PROPERTY',
                        'COMPANY VAULT': 'COMPANY',
                        'GHOST TRADE': 'GHOST'
                    };
                    const candidateTarget = normalizedTargets[String(targetStored).toUpperCase()] || String(targetStored).toUpperCase();
                    const normalizedTarget = ['FACTION', 'PROPERTY', 'COMPANY', 'GHOST'].includes(candidateTarget)
                        ? candidateTarget
                        : 'FACTION';
                    targetSelect.value = normalizedTarget;
                    if (normalizedTarget !== targetStored) {
                        await window.SidekickModules.Core.ChromeStorage.set('quickDeposit_target', normalizedTarget);
                    }
                    console.log('🏦 Loaded target:', normalizedTarget);
                }
                if (ghostRow) ghostRow.style.display = targetSelect.value === 'GHOST' ? 'block' : 'none';
            ...
            if (targetSelect) {
                targetSelect.addEventListener('change', async () => {
                    ...
                    if (ghostRow) ghostRow.style.display = target === 'GHOST' ? 'block' : 'none';
                    try {
