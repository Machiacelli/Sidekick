...
const LockedItemsManagerModule = {
    ...
    processStorePage() {
        ...
        // Accept both Torn's current combined attributes and layouts that expose
        // only one item-ID attribute.
        document.querySelectorAll(
            'li[data-id], li[data-item], li[data-item-id], li[data-itemid]'
        ).forEach(element => {
            if (
        ...
        // React shop layouts may expose the item ID only in the item image.
        // Find the smallest surrounding card that contains that one image and
        // its purchase control, so hiding it removes the whole shop entry.
        document.querySelectorAll(
            'img[src*="/items/"]'
        ).forEach(image => {
            ...
            for (
                ...
                const itemImages = node.querySelectorAll(
                    'img[src*="/items/"]'
                );

                if (itemImages.length !== 1) {
                    if (itemImages.length > 1) break;
                    continue;
                }

                const identifiers = this.getItemIdentifiers(node);
                if (
                    identifiers.uniqueIds.length === 0 &&
                    identifiers.baseIds.length === 0
                ) {
                ...
                const hasPurchaseControl = Boolean(
                    node.querySelector(
                        'button, input[type="submit"], ' +
                        'a[class*="buy" i], [class*="buy" i] button'
                    )
                );

                if (node.tagName === 'LI' || hasPurchaseControl) {
                    fallback = node;
                    break;
                }
            }

            if (fallback) itemSet.add(fallback);
        });

        const items = Array.from(itemSet).filter(element => {
            const identifiers = this.getItemIdentifiers(element);
            return (
                identifiers.uniqueIds.length > 0 ||
                identifiers.baseIds.length > 0
            );
